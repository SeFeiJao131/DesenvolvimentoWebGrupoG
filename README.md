...
> ⚠️ **Aviso importante:** Este projeto foi desenvolvido como trabalho universitário com **auxílio de Inteligência Artificial** (como GitHub Copilot e/ou ferramentas similares) em partes do desenvolvimento. Por esse motivo, o código e as tecnologias utilizadas **podem não refletir integralmente o nível técnico individual dos integrantes do grupo** no momento do desenvolvimento.
...
Valida uma sessão de pagamento Stripe e registra a compra no Firestore. Requer header `Authorization: Bearer <firebase-id-token>`.
...
Cria uma sessão Stripe Checkout para redirecionamento ao checkout hospedado. Requer autenticação via `Authorization: Bearer <firebase-id-token>`.

#### POST /api/criarpagamento
Cria um PaymentIntent Stripe para pagamento com cartão de crédito via Stripe Elements. Requer autenticação.

#### POST /api/criarpix
Gera um PaymentIntent Stripe com método PIX e retorna os dados do QR Code. Requer autenticação.

#### GET /api/verificarpix
Verifica o status de um PaymentIntent PIX via polling. Requer autenticação.

#### GET /api/minhascompras
Retorna o histórico de compras do usuário autenticado, consultando a coleção `compras` no Firestore. Requer autenticação.

#### POST /api/webhookstripe
Recebe e processa eventos do Stripe (ex: `checkout.session.completed`, `payment_intent.succeeded`) para confirmar compras no Firestore. Valida a assinatura do webhook via `STRIPE_WEBHOOK_SECRET`.
...
- Firebase Cloud Function com trigger `onDocumentWritten`
- Endpoint serverless protegido no servidor
- Integração oficial Algolia + Firebase Connectors
...
### Cartão de Crédito
1. O front-end chama `POST /api/criarpagamento` para criar um PaymentIntent
2. O Stripe Elements é montado com a `clientSecret` retornada
3. A confirmação do pagamento ocorre no browser via `stripe.confirmCardPayment()`

### PIX
1. O front-end chama `POST /api/criarpix` para criar um PaymentIntent com método pix
2. A API retorna os dados do QR Code (código copia e cola e imagem base64)
3. O front-end realiza polling em `GET /api/verificarpix` até o pagamento ser confirmado
