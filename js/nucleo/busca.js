...
/* ─── Caminho base dinâmico (raiz vs /paginas/) ──────────────── */
const _emPaginas = window.location.pathname.includes("/paginas/");
const BASE_PAGINAS = _emPaginas ? "" : "paginas/";
...
function htmlAutoComplete(hits, termo) {
  ...
  const itens = hits.map(hit => {
    ...
    return `
      <a class="busca-sugestao-item" href="${BASE_PAGINAS}paginas/produto.html?id=${hit.objectID}">
        <span class="busca-sugestao-nome">
          <i class="fi ${icone}"></i>
          ${nome}
        </span>
