import {
    ...
    function abrirModal(produto = null) {
      ...
      // Limpa nomes de arquivo e barras de progresso
      ...
      // Preenche URL previews se já existem
      if (produto?.urlArquivo) {
        urlsUpload.arquivo = produto.urlArquivo;
        mostrarUrlPreview("arquivo", produto.urlArquivo);
      }
      if (produto?.urlModelo) {
        urlsUpload.modelo = produto.urlModelo;
        mostrarUrlPreview("modelo", produto.urlModelo);
      }
      if (produto?.urlImagem) {
        urlsUpload.imagem = produto.urlImagem;
      }
      ...
      input.addEventListener("change", async () => {
        ...
        uploadTask.on(
          ...
          async () => {
            ...
            // Atualiza campo de URL manual também
            const inputUrl = document.getElementById(`f-${campo === "imagem" ? "imagem" : campo === "arquivo" ? "arquivo" : "modelo"}`);
    ...
    // Configura os três campos de upload
    ...
    // Quando o usuário digita URL manualmente, atualiza o estado
    ...
    document.getElementById("btn-salvar").addEventListener("click", async () => {
      ...
      // Resolve URL da imagem: upload tem prioridade sobre o campo manual
    ...
    window.confirmarDesativar = async (id, nome) => {
      ...
      try {
        ...
        // Remove do índice se desativado, atualiza se reativado
        if (!novoAtivo) {
    ...
    // ── Algolia: sincronização completa ──────────────────────────
    async function sincronizarAlgolia() {
      ...
      try {
        // Busca todos os produtos (sem filtro de ativo para indexar tudo)
        const { getFirestore, collection, getDocs } = await import("https://www.gstatic.com/firebasejs/12.12.0/firebase-firestore.js");
        ...
        for (let i = 0; i < todos.length; i += LOTE) {
          ...
          const url  = `https://AC7XL6FVL6.algolia.net/1/indexes/produtos/batch`;
          const res  = await fetch(url, {
            method: "POST",
            headers: {
              "X-Algolia-Application-Id": "AC7XL6FVL6",
              "X-Algolia-API-Key":        "69484ede99d6d05a2d582dd8aa08591b",
              "Content-Type":             "application/json",
            },
            body: JSON.stringify({
              requests: lote.map(o => ({ action: "updateObject", body: o }))
            }),
          });
        ...
        console.error("Algolia sync erro:", e);
      } finally {
        btn.disabled = false;
    ...
    document.getElementById("btn-sync-algolia").addEventListener("click", sincronizarAlgolia);
    ...
    // ── Lista de e-mails autorizados a acessar o admin ──────────────────────
    // Para adicionar mais admins, basta adicionar o e-mail nesta lista.
    // Futuramente pode ser migrado para Firebase Custom Claims.
    ...
    document.getElementById("btn-login-admin").addEventListener("click", async () => {
      ...
      try {
        ...
        // Verifica imediatamente após login se é admin
    ...
    onAuthStateChanged(auth, (usuario) => {
      if (usuario && ehAdmin(usuario)) {
        // Logado E autorizado — esconde tela de login, mostra painel
        telaLogin.style.display = "none";
        document.getElementById("info-usuario").textContent = usuario.email;
        carregarProdutos();
      } else {
        // Deslogado ou não autorizado — mostra tela de login
        if (usuario && !ehAdmin(usuario)) {
          // Usuário logado mas não é admin — faz logout silencioso
          signOut(auth);
        }
