...
const META = {
  ...
  "textura/superficie":{ titulo: "Superfície",desc: "Superfícies variadas de alta resolução para projetos 3D.",                                      pai: { nome: "Texturas", href: "LayoutTexturas.html" } },
  "textura/tijolo":    { titulo: "Tijolo",    desc: "Texturas de tijolos e alvenaria para visualização e jogos.",                                    pai: { nome: "Texturas", href: "LayoutTexturas.html" } },
  ...
  "hdri/ceu":        { titulo: "Noite",        desc: "HDRIs noturnos com céu estrelado para renders fotorrealistas.",                             pai: { nome: "HDRIs", href: "LayoutHdri.html" } },
...
function criarCard(p) {
  ...
  return `
    <a href="produto.html?id=${p.id}" class="card-categoria">
      ${imagemTag}
      ${badgeNovo}
      ${badgeGratis}
...
async function init() {
  ...
  try {
    ...
    // Filtra pela categoria no frontend
    if (cat) {
      produtos = produtos.filter(p =>
        (p.categorias && p.categorias.includes(cat)) ||
        (p.categoria && p.categoria === cat)
      );
    }
    ...
    if (elCont) elCont.textContent = pluralItem(tipo, produtos.length);
    grade.innerHTML = produtos.map(criarCard).join("");
